...
getRemotePointerInfoFromValue(Value ptr, ConversionPatternRewriter &rewriter) {
  ...
  if (auto addPtrOp = dyn_cast<triton::AddPtrOp>(defOp);
    ...
    if (matchPattern(shardOffset, m_ConstantInt(&shardConst))) {
      ...
      if (!info.constCTAId && matchPattern(shardOffset, m_Constant(&denseConst)) &&
          denseConst.isSplat() && denseConst.getElementType().isInteger(32)) {
        info.constCTAId =
            static_cast<int32_t>(denseConst.getSplatValue<APInt>().getSExtValue());
...
unsigned inferTlePointerVectorSize(Value ptr,
  ...
  unsigned elemsPerThread = std::max<unsigned>(
      1, static_cast<unsigned>(triton::gpu::getTotalElemsPerThread(ptr.getType())));
  unsigned best = 1;
  ...
  for (unsigned axis = 0;
       axis < static_cast<unsigned>(axisInfo->getRank()) &&
       axis < contigPerThread.size();
       ++axis) {
    unsigned contiguity = std::max<unsigned>(
        1, std::min<unsigned>(std::max<int64_t>(1, axisInfo->getContiguity(axis)),
...
unsigned inferTlePointerLayoutVectorHint(Value ptr) {
  ...
  auto dist = dyn_cast<triton::gpu::DistributedEncodingTrait>(tensorTy.getEncoding());
  if (!dist)
    return 1;
  ...
  for (unsigned axis : order) {
    if (axis >= contigPerThread.size())
      continue;
    unsigned candidate =
        std::max<unsigned>(1, std::min<unsigned>(
                                  std::min<unsigned>(maxByType, contigPerThread[axis]),
                                  elemsPerThread));
    best = std::max(best, candidate);
  }
  return best;
