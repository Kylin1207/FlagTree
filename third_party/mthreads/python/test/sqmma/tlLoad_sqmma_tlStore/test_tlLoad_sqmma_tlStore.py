...
@triton.jit
def matmul_kernel_tlLoad_sqmma_tlStore(
    A_ptr,
    B_ptr,
    C_ptr,
    M,
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        if use_mask:
            a = tl.load(A, mask=mask_a)
            b = tl.load(B, mask=mask_b)
        else:
            a = tl.load(A)
            b = tl.load(B)
...
def test_tlLoad_sqmma_tlStore(
    ...
    print(
        f"num_stage: {num_stages}\tM: {M}\tN: {N}\tK: {K}\tBLOCK_M: {BLOCK_M}\tBLOCK_N: {BLOCK_N}\tBLOCK_K: {BLOCK_K}\tdtype: {dtype}\tuse_mask: {use_mask}",
        end="\t\t",
    )
    ...
    kernel = matmul_kernel_tlLoad_sqmma_tlStore[(triton.cdiv(M, BLOCK_M) * triton.cdiv(N, BLOCK_N), 1, 1)](
        A,
        B,
        C,
        M,
...
# test_tlLoad_sqmma_tlStore(1, 32, 32, 32, 32, 32, 32, 16, torch.float16)
...
# test_tmeLoad_sqmma_tlStore(1, 128, 160, 128, 128, 128, 64, 4, torch.float16)
# test_tlLoad_sqmma_tlStore(1, 256, 128, 64, 256, 128, 64, 8, torch.float8_e4m3fn)
