...
from triton.tools.experimental_descriptor import create_2d_tma_descriptor, create_3d_tma_descriptor
...
@triton.jit
def matmul_kernel_tmeLoad_sqmma_tlStore(
    a_desc_ptr,
    b_desc_ptr,
    C_ptr,  #
    M,
    ...
    mask = ((offs_am + tl.arange(0, BLOCK_SIZE_M))[:, None] < M) & ((offs_bn + tl.arange(0, BLOCK_SIZE_N)[None, :]) < N)
...
@pytest.mark.parametrize(
    ...
    [
        (32, 32, 32, 4),
        # (128, 128, 64, 4), # failed maybe due to register spill?
        (64, 64, 32, 16),
        # (256, 256, 64, 16),
        (32, 64, 32, 8),
        (64, 32, 32, 8),
...
def test_tmeLoad_sqmma_tlStore(num_stages, M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, num_warps, dtype):
    device = "musa"
    ...
    kernel = matmul_kernel_tmeLoad_sqmma_tlStore[(triton.cdiv(M, BLOCK_M) * triton.cdiv(N, BLOCK_N), 1, 1)](
        desc_a,
        desc_b,
        C,
        M,
