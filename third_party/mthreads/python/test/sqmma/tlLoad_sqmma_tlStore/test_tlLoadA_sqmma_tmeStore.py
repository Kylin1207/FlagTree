...
@triton.jit
def matmul_kernel_tlLoadA_sqmma_tmeStore(
    A_ptr,
    b_desc_ptr,
    c_desc_ptr,  #
    M,
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        a = tl.load(A)
        b = tl._experimental_descriptor_load(b_desc_ptr, [offs_kb, offs_bn], [BLOCK_SIZE_K, BLOCK_SIZE_N], dtype)
        accumulator = tl.dot(a, b, acc=accumulator)
        A += BLOCK_SIZE_K * 1
        offs_kb += BLOCK_SIZE_K
...
def test_tlLoadA_sqmma_tmeStore(num_stages, M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, num_warps, dtype):
    device = "musa"
    ...
    kernel = matmul_kernel_tlLoadA_sqmma_tmeStore[(triton.cdiv(M, BLOCK_M) * triton.cdiv(N, BLOCK_N), 1, 1)](
        A,
        desc_b,
        desc_c,
        M,
...
# test_tlLoadA_sqmma_tmeStore(1, 32, 32, 32, 32, 32, 32, 4, torch.float16)
# test_tlLoadA_sqmma_tmeStore(1, 32, 32, 32, 32, 32, 32, 4, torch.bfloat16)
# test_tlLoadA_sqmma_tmeStore(1, 32, 32, 32, 32, 32, 32, 4, torch.float8_e4m3fn)
# test_tlLoadA_sqmma_tmeStore(1, 128, 128, 64, 128, 128, 64, 4, torch.float16)
# test_tlLoadA_sqmma_tmeStore(1, 128, 128, 64, 128, 128, 64, 4, torch.bfloat16)
# test_tlLoadA_sqmma_tmeStore(1, 128, 128, 64, 128, 128, 64, 4, torch.float8_e4m3fn)
# test_tlLoadA_sqmma_tmeStore(1, 256, 256, 64, 256, 256, 64, 16, torch.float16)
# test_tlLoadA_sqmma_tmeStore(1, 256, 256, 64, 256, 256, 64, 16, torch.float8_e4m3fn)
